// Solution -- 17.03 Telemetry: the station speaks unprompted
...
static void telemetry_send(void) {
  ...
  buf[2] = CMD_TELEMETRY;
  buf[3] = g_seq;
  buf[4] = (uint8_t)(vdda >> 8);    // big-endian: the wire has one byte order,
  buf[5] = (uint8_t)(vdda & 0xFFu); // and it is not "whatever my CPU does"
  buf[6] = uptime_s > 255u ? (uint8_t)255u : (uint8_t)uptime_s;
  // The CRC covers LEN, CMD and payload -- the format's whole body, not
  // just the part this function happens to find interesting.
  const uint16_t crc = crc16(&buf[1], 6);
  buf[7] = (uint8_t)(crc >> 8);
