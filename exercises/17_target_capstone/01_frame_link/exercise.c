// =============================================================================
//  17.01 -- The frame link: bytes with a spine
// =============================================================================
//
//  The capstone begins. Over the next four exercises the chapter-14 command
//  station moves onto real copper: frames arrive through USART2's RX pin
//  (driven by your Mac through the ST-Link bridge -- `./mec flash` feeds
//  this exercise's feed.txt to the board when the test asks), an interrupt
//  drops the bytes into an SPSC ring (12.04, now with a genuine ISR), and a
//  byte-at-a-time state machine (13.01's discipline) rebuilds frames on the
//  other side.
//
//  THE WIRE FORMAT, which chapters 14 and 17 share:
//
//    0xAA | LEN | CMD | payload[LEN] | CRC16 hi | CRC16 lo
//
//  LEN counts payload bytes only, 32 at most. The CRC (CCITT-FALSE: poly
//  0x1021, init 0xFFFF -- check value over "123456789" is 0x29B1) covers
//  LEN, CMD and payload. A serial line guarantees nothing: bytes vanish
//  (overrun), arrive corrupted, or start mid-stream. The parser's job is to
//  deliver only whole, verified frames -- and to SURVIVE everything else.
//
//  Survival is the point of both starter bugs:
//
//  1. THE LENGTH THAT BELIEVES ANYTHING. A stray 0xAA in line noise makes
//     the parser read the NEXT garbage byte as LEN. If that byte is 0xFF,
//     an unchecked parser settles in to collect 255 payload bytes -- eating
//     every real frame that follows for a quarter of a kilobyte. Validate
//     LEN the moment it arrives; refuse and go back to hunting.
//
//  2. THE RESYNC THAT ISN'T. After a CRC failure the starter helpfully
//     assumes the next byte is the LEN of another frame. It is not -- CRC
//     failure means you do not know WHERE you are in the stream. The only
//     honest state after any verdict, good or bad, is HUNT_SOF. (Watch the
//     scripted feed prove it: the frame after the corrupted one gets eaten
//     whole.)
//
//  The RX ring, the ISR (note the ORE overrun handling -- a w1c clear from
//  11.04), the CRC and the tick are given. The parser is yours.
//
//  RUN IT
//    ./mec flash 17_01
//
...
enum rx_event parser_feed(struct parser *p, uint8_t byte, struct frame *out) {
  ...
  case WANT_LEN:
    // TODO: any byte is believed. FRAME_MAX_PAYLOAD would like a word.
    p->work.len = byte;
  ...
  case WANT_CRC_LO: {
    ...
    if (received != p->crc) {
      // TODO: "frames come back to back, so LEN is surely next" -- is it?
      p->state = WANT_LEN;
      return RX_BAD_CRC;
    }
    p->state = HUNT_SOF;
