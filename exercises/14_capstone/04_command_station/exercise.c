// =============================================================================
//  14.04 -- The command station
// =============================================================================
//
//  The assembly. Frames come in (14.02), commands dispatch through a
//  function-pointer table (03.06), replies go out through a byte ring
//  (08.06) carrying 14.01's CRC, and CMD_READ_SENSOR reports 14.03's
//  filtered value. One static module instance with an init/reset entry
//  point (09.07). Nothing here is new; the exercise is watching the pieces
//  compose -- and noticing how each chapter's discipline protects the next:
//
//   - The parser has already CRC-checked and bounded everything before a
//     frame reaches dispatch, so handlers trust `f->len` -- but NOT
//     `f->cmd`. A valid frame can still carry a command we never defined.
//     The dispatch table is indexed BY that byte, and an index off a wire
//     gets bounds-checked before it touches an array, always (chapter 04,
//     chapter 03 -- same rule, third costume). Note slot [0]: there is no
//     command zero, so even an in-range index needs its NULL check.
//
//   - Replies are built by one function, `send_frame`, because framing
//     rules live in exactly one place. Its LEN field counts PAYLOAD BYTES
//     ONLY -- the starter counts the CMD byte too, a misreading the other
//     side's parser will reject on every single reply. The tests compare
//     raw reply bytes against oracle-verified frames, so the off-by-one is
//     visible immediately.
//
//  On the real board (chapter 17) this exact module runs against a real
//  UART; the only thing that changes is who calls station_feed().
//
//  TASK
//    Two TODOs: bounds-check the dispatch, fix the LEN field. Do not
//    change the tests. (Expect the unknown-command test to CRASH before
//    you fix dispatch -- an out-of-range index into a table of function
//    pointers is a jump to wherever; run the asan preset to see it named.)
//
//  RUN IT
//    ./mec test 14_04
//
...
static void send_frame(uint8_t cmd, const uint8_t *payload, uint8_t len) {
  ...
  // TODO: LEN counts payload bytes only. "Payload plus the command byte"
  // is the other, wrong reading of the spec -- and this is it.
  ring_put(&st.out, (uint8_t)(len + 1u));
  ring_put(&st.out, cmd);
  uint16_t crc = crc16_step(CRC16_INIT, (uint8_t)(len + 1u));
...
static void dispatch(const struct frame *f) {
  // TODO: f->cmd came off a wire. 0x55 indexes 80 entries past the end of
  // a 5-entry table and CALLS whatever bytes it finds there. Check the
  // range, check for the NULL slots, NAK and count what does not dispatch.
  handlers[f->cmd](f);
}
